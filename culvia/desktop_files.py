...
def _folder_picker_command(prompt: str, platform: str, which: Any = None) -> tuple[list[str], dict[str, str] | None]:
    if platform == "darwin":
    ...
    if platform.startswith("linux"):
        if _command_path("zenity", which):
            return ["zenity", "--file-selection", "--directory", "--title", prompt], None
